...
const templateFiles = fs
  .readdirSync(path.join(CLI_PATH, 'template-angular-tailwind'))
...
it('successfully scaffolds a project based on Angular Tailwind starter template', () => {
  const { stdout } = run([projectName, '--template', 'angular-tailwind'], {
    cwd: __dirname,
  })
...
it('works with the -t alias', () => {
  const { stdout } = run([projectName, '-t', 'angular-tailwind'], {
    cwd: __dirname,
  })
  const generatedFiles = fs.readdirSync(genPath).sort()
...
it('return help usage how to use create-charada', () => {
  const { stdout } = run(['--help'], { cwd: __dirname })
  const message = 'Usage: create-charada [OPTION]... [DIRECTORY]'
  expect(stdout).toContain(message)
})

it('return help usage how to use create-charada with -h alias', () => {
